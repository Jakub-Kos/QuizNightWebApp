...
import { ArrowLeft, ArrowRight, Check, X, Edit3, Hash, Clock } from "lucide-react";
...
export default function QuestionScreen({ roundData, mode, onBack, isPresenter, t = {} }) {
  ...
  // Ask for current question state when component mounts
  useEffect(() => {
  ...
  const currentRaw = qIndex >= 0 ? roundData.questions[qIndex] : null;

  const question = currentRaw ? {
    id: currentRaw[1],
    text: currentRaw[2],
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      // Allow controls from ANY active window now
  ...
  const next = () => {
    ...
    if (step < maxSteps - 1) {
      ...
      if (qIndex < roundData.questions.length - 1) {
  ...
  const prev = () => {
    if (step > 0) {
      ...
      if (qIndex > 0) {
        setQIndex(prev => prev - 1);
        const prevType = roundData.questions[qIndex-1][3] ? String(roundData.questions[qIndex-1][3]).trim() : "Written";
                      ...
                      {qIndex === -1 ? (
                          ...
                          <div className="flex flex-col h-full">
                              <div className="flex justify-between items-end mb-6 border-b border-white/10 pb-4">
                                  <h2 className="text-4xl font-black text-white">Question {qIndex + 1} / {roundData.questions.length}</h2>
                       ...
                       <div className="flex-1 flex flex-col justify-center items-center text-center px-10">
                            ...
                            <h3 className="text-3xl font-bold text-white/60">
                                {qIndex === -1 ? "Reveal Big Question 1 Number" :
                                 ...
                                 qIndex < roundData.questions.length - 1 ? `Proceed to Question ${qIndex + 2}` :
  ...
  const showOptions = showQuestion && (question?.type === "ABCD" || question?.type === "Yes/No" || question?.type === "Sort");
    ...
    <div className="h-screen w-full bg-[#050505] relative overflow-hidden flex flex-col font-['League_Spartan']">
                                ...
                                <div className="flex flex-col items-center">
                                    {showAnswer ? (
                                        ...
                                        <div className="flex items-center gap-6 px-16 py-8 rounded-[3rem] bg-white/5 border border-white/10 text-white/50 animate-pulse shadow-inner">
                                            {question.type === "Numeric" ? <Hash size={40} /> : <Edit3 size={40} />}
        ...
        <div className="h-[10vh] border-t border-white/5 bg-black/50 backdrop-blur-md flex items-center justify-between px-10 relative z-30">
             <div className="flex items-center gap-4 text-white/40 font-mono text-xs uppercase tracking-widest font-bold">
                ...
                <span>Q {qIndex + 1} / {roundData.questions.length}</span>
